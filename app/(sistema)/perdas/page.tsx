...
import { percasAPI, casasAPI, Perca, Casa } from '@/lib/api'
import { useAuth } from '@/components/AuthProvider'
import { Combobox } from '@headlessui/react'

export default function PercasPage() {
  const { token } = useAuth()
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const [percasData, casasData] = await Promise.all([
        percasAPI.getAll(),
        casasAPI.getAll()
      ])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await percasAPI.create(dataParaApi)
      setFormData({ casaId: 0, valor: '', casaSearch: '' })
      loadData()
    } catch (error) {
      console.error('Erro ao registrar perda:', error)
      alert('Falha ao registrar a perda.')
    }
  }

  const handleDelete = async (id: number) => {
    if (confirm('Tem certeza que deseja deletar este registro de perda?')) {
      try {
        await percasAPI.delete(id)
        loadData()
      } catch (error) {
  ...
  const totalPerdido = percas.reduce((sum, perca) => sum + perca.valor, 0);
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Registro de Perdas</h1>
          <p className="text-gray-600">Adicione perdas avulsas que não se enquadram em arbitragens.</p>
        </div>
      </div>
      
      {/* Card de Total Perdido */}
      <div className="mb-6">
        <div className="card">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-2xl font-bold text-red-600">
                        -R$ {totalPerdido.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </p>
                </div>
      ...
      {/* Formulário de Nova Perda */}
      <div className="card mb-6">
        <h2 className="text-xl font-semibold mb-4">Adicionar Nova Perda</h2>
        <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-4 items-end">
          ...
          <div className="flex-1">
            <label className="block text-sm font-medium text-gray-700 mb-1">Valor Perdido</label>
            <input
              type="text"
              name="valor"
      ...
      {/* Tabela de Percas */}
      <div className="card">
        <h2 className="text-xl font-semibold mb-4">Histórico de Perdas</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody>
              {percas.map((perca) => (
                <tr key={perca.id} className="border-b hover:bg-gray-50">
                  <td className="p-3">{perca.casa.nome}</td>
                  <td className="p-3 text-red-600">
                    -R$ {perca.valor.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </td>
                  <td className="p-3">{new Date(perca.data).toLocaleDateString('pt-BR')}</td>
                  <td className="p-3 text-right">
                    <button
                      onClick={() => handleDelete(perca.id)}
          ...
          {percas.length === 0 && (
            <p className="text-center text-gray-500 py-6">Nenhum registro de perda encontrado.</p>
          )}
        </div>
      </div>
    </div>
